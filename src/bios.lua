...
do
  ...
  local function boot_invoke(address, method, ...)
    ...
    else return table.unpack(result, 2, result.n) end
  end

  -- backwards compatibility, may remove later
  local eeprom = component.list("eeprom")()
  computer.getBootAddress = function() return boot_invoke(eeprom, "getData") end
  computer.setBootAddress = function(address) return boot_invoke(eeprom, "setData", address) end
  ...
  if not init then
    ...
    for address in component.list("filesystem") do
      ...
      if init then computer.setBootAddress(address) break
      end
    end
  end
